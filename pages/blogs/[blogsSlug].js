...
export async function getServerSideProps(context) {
    const post5 = await fetch('https://yomablogs.onrender.com/api/posts')
    const posts5 = await post5.json()

    const cat = await fetch('https://yomablogs.onrender.com/api/categories')
    const cats = await cat.json()


    const posturl = context.params.blogsSlug;
    const res = await fetch(`https://yomablogs.onrender.com/api/post/${posturl}`)
    const posts = await res.json()

    const getfeedback = await fetch(`https://yomablogs.onrender.com/api/posts/feed/${posturl}`)
...
const submitFeedback = async (event) => {
    ...
    xhr.open('POST', 'https://yomablogs.onrender.com/api/feedback/');
