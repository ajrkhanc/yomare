...
export async function getServerSideProps() {
    const res = await fetch('https://yomablogs.onrender.com/uploads/proanto/resume/state.json')
...
export default function PrivatePage(State, currentState) {
    ...
    const uploadToServer = async (event) => {
        ...
        console.log(random1)
        const response = await fetch("https://yomablogs.onrender.com/api/resume", {
            method: "POST",
            body
        });
    };
    ...
    const submitFeedback = async (event) => {
        ...
        xhr.open('POST', 'https://yomablogs.onrender.com/api/jobform/');
